...
We change it to box plot and try to put all the school and its performance in the graph. In this way, the graph looks much more crowded but we are able to see all the school's performance along with its ID.#need a little bit zoom in

```{r}
#| code-fold: true
options(repr.plot.height = 100)

p4 <- ggplot(new_data, aes(x = as.factor(CNTSCHID), y = Math)) +
  geom_boxplot(width = 0.3, position = position_dodge(width = 0.2), lwd = 0.5, outlier.shape = NA) +
  stat_summary(
    fun = mean,
    geom = "point",
    shape = 18,
  ...
  coord_flip() +
  theme_minimal(base_size = 3.5)

p5 <- ggplot(new_data, aes(x = as.factor(CNTSCHID), y = Science)) +
  geom_boxplot(width = 0.3, position = position_dodge(width = 0.2), lwd = 0.5, outlier.shape = NA) +
  stat_summary(
    fun = mean,
    geom = "point",
...
p6 <- ggplot(new_data, aes(x = as.factor(CNTSCHID), y = Reading)) +
  geom_boxplot(width = 0.3, position = position_dodge(width = 0.2), lwd = 0.5, outlier.shape = NA) +
  stat_summary(
    fun = mean,
    geom = "point",
...
(p4|p5|p6)+plot_annotation(tag_levels = 'I')
